// EHIJING_SMASH_DNDQ2DZ.cc
//
// Rivet analysis to produce (1/Nevt) dN/(dQ2 dz_h) vs Q2 in fixed z_h slices
...
//   /EHIJING_SMASH_DNDQ2DZ/dN_dQ2dz_pip_zh0p2_0p3
//   /EHIJING_SMASH_DNDQ2DZ/dN_dQ2dz_pim_zh0p3_0p4
//   /EHIJING_SMASH_DNDQ2DZ/dN_dQ2dz_kp_zh0p4_0p6
//   /EHIJING_SMASH_DNDQ2DZ/dN_dQ2dz_km_zh0p6_0p8
//   ... similarly for pim, kp, km
//
// Normalization in finalize:
//  - divide by Nev = sumW()
//  - convert to density in Q2 and z_h: scale by 1/(dQ2 * dz)
...
namespace Rivet {
    ...
    class EHIJING_SMASH_DNDQ2DZ : public Analysis {

    public:

        RIVET_DEFAULT_ANALYSIS_CTOR(EHIJING_SMASH_DNDQ2DZ);
        ...
        void init() override {
            ...
            // Q2 axis config
            _Q2Edges = {1.0, 1.25, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0, 5.0, 15.0};
            if (_Q2Edges.size() < 2) {
                throw UserError("Q2 bin edge list must contain at least two entries.");
            }
            for (size_t i = 1; i < _Q2Edges.size(); ++i) {
                if (!std::isfinite(_Q2Edges[i-1]) || !std::isfinite(_Q2Edges[i]) || _Q2Edges[i] <= _Q2Edges[i-1]) {
                    throw UserError("Q2 bin edges must be strictly increasing and finite.");
                }
            }
        ...
        void analyze(const Event& event) override {
            ...
            // Event-level Q2
            const double Q2 = eventKinematics.Q2;
            if (!std::isfinite(Q2)) {
                _nEventsVetoed++;
                vetoEvent;
            }
            ...
            for (const Particle& particle : fs.particles()) {
                ...
                // Fill Q2 histogram for this z_h slice and species
                _h[is][iz]->fill(Q2, 1.0);
                _nFilled++;
            }
        } // analyze()
        ...
        void finalize() override {
            ...
            // Convert to density: (1/Nev) dN/(dQ2 dz)
            for (size_t is = 0; is < NSPEC; ++is) {
                for (size_t iz = 0; iz < NZ; ++iz) {
    ...
    private:
        ...
        std::vector<double> _Q2Edges = {1.0, 1.25, 1.5, 1.75, 2.0, 2.25, 2.5, 3.0, 5.0, 15.0};
    ...
    constexpr double EHIJING_SMASH_DNDQ2DZ::zEdges[NZ+1];

    RIVET_DECLARE_PLUGIN(EHIJING_SMASH_DNDQ2DZ);
